...
class Area {
  ...
  update(time, players, worldPos) {
    ...
    for (const entityType in this.entities) {
      ...
      if (entityType[0] == "g"){
        ...
        //     if (entitying1 === entitying2) continue;
        //     console.log(entitying1, entitying2);
        //   }
        // }
        const powring = 0.1;
        const sprlen = 6;
        this.entities[entityType].forEach (entitying1 => {
          this.entities[entityType].forEach (entitying2 => {
            if (entitying1 === entitying2) {
              // console.log("ADSADSADSADSADS");
            }
            else {
              // console.log(entitying1, entitying2);
              const initing = new Vector(entitying2.pos.x - entitying1.pos.x, entitying2.pos.y - entitying1.pos.y);
              const distting = Math.sqrt(Math.pow(initing.x, 2) + Math.pow(initing.y, 2));
              const disto2 = new Vector(initing.x / distting * sprlen, initing.y / distting * sprlen);
              const adjustin = new Vector(disto2.x - initing.x, disto2.y - initing.y);
              if (entitying1.freeze > 0) {} else {
                entitying1.vel.x -= adjustin.x * time / (1000 / 30) * powring / 2;
                entitying1.vel.y -= adjustin.y * time / (1000 / 30) * powring / 2;
              }
              if (entitying2.freeze > 0) {} else {
                entitying2.vel.x += adjustin.x * time / (1000 / 30) * powring / 2;
                entitying2.vel.y += adjustin.y * time / (1000 / 30) * powring / 2;
              }
            }
          });
        });
  ...
  createEnemy(enemyType, posX, posY, radius, speed, angle, preset, auraRadius, j, count) {
    switch (enemyType) {
      ...
      case "graphed":
        return new Graphed(new Vector(posX, posY), radius / 32, speed, angle);
      default:
